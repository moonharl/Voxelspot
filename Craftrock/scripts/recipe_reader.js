...
var unsorted = JSON.parse(JSON.stringify(eval(list)));

var sorted = '[]';
var newSort = JSON.parse(sorted);
for(var lk = 0; lk < unsorted.length; lk++){
  if(unsorted[lk].genre == type){
    newSort.push(unsorted[lk]);
  }
}
const recipe = JSON.parse(JSON.stringify(newSort));
